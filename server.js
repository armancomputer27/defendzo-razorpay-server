...
app.post("/create-mandate", async (req, res) => {
  try {
    const { customerName, customerPhone, customerEmail, amount, description, type } = req.body;
    ...
    const payload = {
      ...
      reference_id: mandate_,
      description: description || "E-Mandate Test Link",
      customer: { name: customerName, contact: customerPhone, email: customerEmail || "test@example.com" },
